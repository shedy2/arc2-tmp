...
class ARC2_SPARQLParser extends ARC2_TurtleParser {
    ...
    function xGraphPatternNotTriples($v) {
        ...
        if ((list($sub_r, $sub_v) = $this->xMinusGraphPattern($v)) && $sub_r) {
            return array($sub_r, $sub_v);
        }
    ...
    function xMinusGraphPattern($v) {
        if ($sub_r = $this->x('MINUS', $v)) {
            $sub_v = $sub_r[1];
            if ((list($sub_r, $sub_v) = $this->xGroupGraphPattern($sub_v)) && $sub_r) {
                return array(array('type' => 'minus', 'patterns' => $sub_r['patterns']), $sub_v);
            }
            $this->addError('Missing or invalid Group Graph Pattern after MINUS');
        }
        return array(0, $v);
    }
